...
async function getRecipes() {
  ...
  let recipe_from_storage = []
  let parsed = JSON.parse(localStorage.getItem('recipes'));
  if(parsed != null){
    parsed.forEach((recipe) => {
      recipe_from_storage.push(recipe);
    });
  }
  // return each_recipe;
  /**************************/
  ...
  return new Promise(async (resolve, reject) => {
    for(var i = 0; i < RECIPE_URLS.length; i++){
      try {
        let fetch_url = await fetch(RECIPE_URLS[i])
        let url_json = await fetch_url.json()
        recipes_fetched.push(url_json)
        if(i == RECIPE_URLS.length-1){
          saveRecipesToStorage(recipes_fetched);
          resolve(recipes_fetched);
        }
      } catch(error) {
        console.error(error);
        reject(error);
